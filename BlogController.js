...
function updateBlogById(request,response){
    request.on('data',function(data){
        var data = JSON.parse(data.toString())
         BlogDao.updateBlogById(data.id,data.content,timeUtil.getNow(),function(result){
            response.send({status:'success',msg:'修改成功',result:1})
         })
    })
}
...
path.set('/updateBlogById',updateBlogById)
